...
echo "$(date): Attempting to stop scalping process..." >> /var/log/gapup.log
...
# Log completion
echo "$(date): Stop script completed" >> /var/log/gapup.log

# Optional: Check if process is still running
if pgrep -f "main.py run" > /dev/null; then
    echo "$(date): WARNING - Process still running!" >> /var/log/gapup.log
else
    echo "$(date): Process successfully stopped" >> /var/log/gapup.log
fi